...
<body>
    ...
    def generate_html(self, analysis: Dict[str, Any]) -> str:
        ...
        if analysis.get('depot_statements'):
            ...
            if all_statements:
                html_parts.append("### Alle Depotabschlüsse (chronologisch)")
                html_parts.append("| Typ | Stichtag | Dokumentdatum | Stück | ISIN | Depotbestand (EUR) | Dokument |")
                html_parts.append("|-----|----------|---------------|-------|------|-------------------|----------|")
                ...
                for statement in all_statements:
                    ...
                    html_parts.append(f"| {type_display} | {balance_date} | {doc_date} | {shares_str} | {isin_str} | {closing} | {doc_link} |")
            ...
            if total_statements > 0:
                ...
                html_parts.append(f"*Gesamtzeitraum: {first_date_german} bis {last_date_german}*")
        
        # Abschnitt 3: Jährliche Kostenanalyse (MiFID II)
        if analysis.get('cost_information'):
            html_parts.append(f"\n## Jährliche Kostenanalyse (MiFID II)\n")
            html_parts.append("Gesetzlich vorgeschriebene Kostenaufstellung gemäß Art. 50 der Verordnung (EU) 2017/565:")
            ...
            for year in years:
                ...
                html_parts.append(f"| {year} | {doc_date} | {service_costs_net} | {depot_fees_net} | {subtotal_net} | {total_vat} | {total_costs} | {volume} | {avg_depot} | {cost_quote} | {doc_link} |")
            ...
            if len(years) > 0:
                ...
                if total_all_volume > 0:
                    ...
                    html_parts.append(f"*Gesamtes Handelsvolumen: {volume_str} €*")
                ...
                if quotes:
                    ...
                    html_parts.append(f"*Durchschnittliche jährliche Kostenquote: {avg_quote:.2f}%*")
                
                # Umfassende Erläuterungen zur Kostenstruktur
                html_parts.append("\n### Erläuterungen zur Kostenberechnung:")
                html_parts.append("\n**Kostenzusammensetzung:**")
                html_parts.append("- **Dienstleistungskosten:** Beinhalten alle Kosten für Wertpapiergeschäfte (Kauf/Verkauf), Ordergebühren und sonstige Transaktionskosten")
                html_parts.append("- **Depotentgelte:** Jährliche Verwahrungsgebühren für die Führung des Depotkontos (übergreifende Kosten)")
                html_parts.append("- **Zwischensumme (netto):** Summe aus Dienstleistungskosten und Depotentgelten ohne Umsatzsteuer")
                html_parts.append("- **USt (19%):** Gesetzliche Umsatzsteuer auf alle Kosten (sowohl Dienstleistungs- als auch Depotentgelte)")
                html_parts.append("- **Gesamtkosten (brutto):** Vollständige Kosten inklusive 19% Umsatzsteuer")
                
                html_parts.append("\n**Wichtige Hinweise:**")
                html_parts.append("- ✓ Die Depotentgelte sind **in den Gesamtkosten enthalten** (Dienstleistungskosten + Depotentgelte = Gesamtkosten)")
                html_parts.append("- ✓ Alle Beträge in den verlinkten Dokumenten sind **Bruttobeträge** (bereits inkl. 19% USt)")
                html_parts.append("- ✓ Die Nettobeträge werden berechnet: Netto = Brutto ÷ 1,19")
                html_parts.append("- ✓ Die USt wird berechnet: USt = Brutto - Netto")
                ...
                if '2021' in analysis['cost_information']:
                    ...
                    html_parts.append(f"- Dienstleistungskosten (brutto): {service_gross:.2f} € ÷ 1,19 = {service_gross/1.19:.2f} € (netto)")
                    html_parts.append(f"- Depotentgelte (brutto): {depot_gross:.2f} € ÷ 1,19 = {depot_gross/1.19:.2f} € (netto)")
                    html_parts.append(f"- **Gesamtkosten: {service_gross:.2f} € + {depot_gross:.2f} € = {total_gross:.2f} € (brutto)**")
        ...
        html_parts.extend(doc_table_lines)
        ...
        if categories['Kostenaufstellung']:
            ...
            for trans_type, count in categories['Kostenaufstellung']:
                html_parts.append(f"- **{trans_type}:** {count}")
        ...
        if failed_count > 0:
            ...
            html_parts.append("Diese Dokumente erfordern manuelle Überprüfung oder erneute Verarbeitung.")
        ...
        if analysis['isin_groups']:
            ...
            for isin, trans_list in analysis['isin_groups'].items():
                ...
                if isin == "DE0001234567":
                    ...
                    html_parts.append(f"- ⚠️ **WARNUNG: Dies ist eine Test-ISIN und sollte nicht in Produktivdaten vorkommen!**")
                else:
                    html_parts.append(f"\n### {isin}")
                html_parts.append(f"- **Anzahl Transaktionen:** {len(trans_list)}")
                ...
                if verkauf_summe > 0:
                    html_parts.append(f"- **Verkaufssumme:** {verkauf_summe:,.2f} EUR")
                if kauf_summe > 0:
                    html_parts.append(f"- **Kaufsumme:** {kauf_summe:,.2f} EUR")
        
        # Abschnitt 7: Detaillierter Transaktionsverlauf
        html_parts.append(f"\n## Transaktionsverlauf (Detailliert)\n")
        
        # Legende der Spalten
        html_parts.append("### Legende der Spalten:")
        html_parts.append("- **Nr.**: Laufende Nummer der Transaktion")
        html_parts.append("- **Wertst.**: Wertstellungsdatum/Valuta (Settlement)")
        html_parts.append("- **Handel**: Handelstag/Schlusstag (Ausführung)")
        html_parts.append("- **Dokument**: Dokumenterstellungsdatum")
        html_parts.append("- **G/V Akt.**: Gewinn/Verlust aus Aktienverkäufen (EUR)")
        html_parts.append("- **Kum. Akt.**: Kumulierter Gewinn/Verlust Aktien (EUR)")
        html_parts.append("- **G/V N-Akt.**: Gewinn/Verlust aus Nicht-Aktien (Derivate, Zertifikate, strukturierte Produkte) (EUR)")
        html_parts.append("- **Kum. N-Akt.**: Kumulierter Gewinn/Verlust Nicht-Aktien (EUR)")
        html_parts.append("- **G/V Ges.**: Gewinn/Verlust Gesamt (EUR)")
        html_parts.append("- **Kum. Ges.**: Kumulierter Gewinn/Verlust Gesamt (EUR)")
        html_parts.append("")
        
        # Tabellenkopf (verkürzt wegen Platzmangel)
        html_parts.append("| Nr. | Wertst. | Handel | Dokument | Typ | ISIN | Stück | Kurs | Kurswert | Geb. | USt | Ausmach. | G/V Akt. | Kum. Akt. | G/V N-Akt. | Kum. N-Akt. | G/V Ges. | Kum. Ges. | Dokument |")
        html_parts.append("|-----|---------|--------|----------|-----|------|-------|------|----------|------|-----|----------|----------|-----------|------------|-------------|----------|-----------|----------|")
        ...
        for trans in analysis['transactions']:
            ...
            # Extrahiere alle drei Datumsangaben mit Fallback-Logik
            # Wertstellung (wichtigstes Datum für Buchhaltung)
            ...
            if not value_date:
                # Wenn keine Wertstellung, nutze Handelstag oder Dokumentdatum
                value_date = execution_date or trans.get('stichtag') or doc_date
            if not execution_date:
                # Wenn kein Handelstag, nutze Stichtag oder Dokumentdatum
                execution_date = trans.get('stichtag') or doc_date
            
            # Formatiere für Anzeige (deutsches Format)
            ...
            # Bestimme Zeilenstil basierend auf Transaktionstyp
            row_style = ""
            use_html_row = False  # Flag für HTML-Zeilen
            
            if 'Depotabschluss' in trans_type:
                # Schwarzer Hintergrund für Depotabschlüsse
                row_style = 'background-color: #000000; color: white;'
                use_html_row = True
            elif 'Verkauf' in trans_type and 'Ausführungsanzeige' not in trans_type:
                # Verkäufe mit G/V-basierter Färbung
                if trans.get('profit_loss') is not None:
                    if trans['profit_loss'] > 0:
                        # Grün für Gewinn
                        row_style = 'background-color: #28a745; color: white;'
                    else:
                        # Rot für Verlust
                        row_style = 'background-color: #dc3545; color: white;'
                    use_html_row = True
            elif trans_type in ['Kauf', 'Orderabrechnung-Kauf']:
                # Normaler Stil für Käufe (keine spezielle Färbung)
                row_style = ''
            else:
                # Grauer Text für sonstige Transaktionen (Vormerkungen, Stornierungen, etc.)
                row_style = 'color: #6c757d;'
                use_html_row = True
            ...
            if use_html_row and row_style:
                # HTML-Zeile mit Stil
                html_parts.append(f'<tr style="{row_style}">')
                html_parts.append(f'<td>{transaction_number}</td>')
                html_parts.append(f'<td>{value_date_str}</td>')
                html_parts.append(f'<td>{execution_date_str}</td>')
                html_parts.append(f'<td>{doc_date_str}</td>')
                html_parts.append(f'<td>{display_type}</td>')
                html_parts.append(f'<td>{isin_display}</td>')
                html_parts.append(f'<td>{shares}</td>')
                html_parts.append(f'<td>{price_str}</td>')
                html_parts.append(f'<td>{kurswert_netto}</td>')
                html_parts.append(f'<td>{gebühren_netto_str}</td>')
                html_parts.append(f'<td>{ust_str}</td>')
                html_parts.append(f'<td>{ausmachend_brutto}</td>')
                html_parts.append(f'<td>{gv_aktien_str}</td>')
                html_parts.append(f'<td>{kum_aktien_str}</td>')
                html_parts.append(f'<td>{gv_non_aktien_str}</td>')
                html_parts.append(f'<td>{kum_non_aktien_str}</td>')
                html_parts.append(f'<td>{gv_gesamt_str}</td>')
                html_parts.append(f'<td>{kum_gesamt_str}</td>')
                html_parts.append(f'<td>{pdf_link}</td>')
                html_parts.append('</tr>')
            else:
                # Standard Markdown-Zeile
                html_parts.append(f"| {transaction_number} | {value_date_str} | {execution_date_str} | {doc_date_str} | {display_type} | {isin_display} | {shares} | {price_str} | {kurswert_netto} | {gebühren_netto_str} | {ust_str} | {ausmachend_brutto} | {gv_aktien_str} | {kum_aktien_str} | {gv_non_aktien_str} | {kum_non_aktien_str} | {gv_gesamt_str} | {kum_gesamt_str} | {pdf_link} |")
        ...
        if depotabschluss_count > 0:
            html_parts.append(f"*- Depotabschlüsse: {depotabschluss_count}*")
        if orderabrechnung_count > 0:
            html_parts.append(f"*- Orderabrechnungen: {orderabrechnung_count}*")
        if verkauf_count > 0:
            html_parts.append(f"*- Verkäufe (sonstige): {verkauf_count}*")
        if kauf_count > 0:
            html_parts.append(f"*- Käufe (sonstige): {kauf_count}*")
        if failed_count > 0:
            html_parts.append(f"⚠️ **{failed_count} fehlgeschlagene Extraktionen**")
        if unknown_count > 0:
            html_parts.append(f"⚠️ **{unknown_count} unbekannte Dokumente**")
        ...
        html_parts.append(f"- **Aktien Gesamt:** {cumulative_stock_str} EUR (Kumuliert)")
        html_parts.append(f"- **Nicht-Aktien Gesamt:** {cumulative_non_stock_str} EUR (Kumuliert)")
        html_parts.append(f"- **Gesamtergebnis:** {cumulative_total_str} EUR")
        
        # Steuerliche Hinweise
        html_parts.append(f"\n### Steuerliche Hinweise:")
        html_parts.append("- **Aktiengewinne:** Unterliegen der Abgeltungssteuer (25% + SolZ + ggf. KiSt)")
        html_parts.append("- **Nicht-Aktien (Derivate/Zertifikate):** Verluste nur mit Gewinnen aus Termingeschäften verrechenbar (§ 20 Abs. 6 EStG)")
        html_parts.append("- **Verlustverrechnungstöpfe:** Aktien- und Derivatverluste werden steuerlich getrennt behandelt")
        
        # Farbcodierung-Legende
        html_parts.append(f"\n### Farbcodierung:")
        html_parts.append("- 🟢 **Grüne Schrift**: Gewinnbringende Verkäufe")
        html_parts.append("- 🔴 **Rote Schrift**: Verlustbehaftete Verkäufe")
        html_parts.append("- **Grüner Hintergrund**: Verkaufstransaktion mit Gewinn")
        html_parts.append("- **Roter Hintergrund**: Verkaufstransaktion mit Verlust")
        ...
        if monthly_groups:
            html_parts.append("| Monat | Anzahl Transaktionen | Gesamtvolumen (EUR) |")
            html_parts.append("|-------|---------------------|---------------------|")
            
            for month in sorted(monthly_groups.keys()):
                data = monthly_groups[month]
                html_parts.append(f"| {month} | {data['count']} | {data['volume']:,.2f} |")
