...
"""
        ...
        # Abschnitt 2: Depotabschluss-Übersicht 
        ...
        md.append("- **Wertst.**: Wertstellungsdatum/Valuta (Settlement)")
        md.append("- **Handel**: Handelstag/Schlusstag (Ausführung)")
        md.append("- **Dokument**: Dokumenterstellungsdatum")
        ...
        md.append("| Nr. | Wertst. | Handel | Dokument | Typ | ISIN | Stück | Kurs | Kurswert | Geb. | USt | Ausmach. | G/V Akt. | Kum. Akt. | G/V N-Akt. | Kum. N-Akt. | G/V Ges. | Kum. Ges. | Dokument |")
        md.append("|-----|---------|--------|----------|-----|------|-------|------|----------|------|-----|----------|----------|-----------|------------|-------------|----------|-----------|----------|")
        ...
        for trans in analysis['transactions']:
            ...
            if use_html_row and row_style:
                # HTML-Zeile mit Stil
                md.append(f'<tr style="{row_style}">')
                md.append(f'<td>{transaction_number}</td>')
                md.append(f'<td>{value_date_str}</td>')
                md.append(f'<td>{execution_date_str}</td>')
                md.append(f'<td>{doc_date_str}</td>')
                md.append(f'<td>{display_type}</td>')
                md.append(f'<td>{isin_display}</td>')
                md.append(f'<td>{shares}</td>')
                md.append(f'<td>{price_str}</td>')
                md.append(f'<td>{kurswert_netto}</td>')
                md.append(f'<td>{gebühren_netto_str}</td>')
                md.append(f'<td>{ust_str}</td>')
                md.append(f'<td>{ausmachend_brutto}</td>')
                md.append(f'<td>{gv_aktien_str}</td>')
                md.append(f'<td>{kum_aktien_str}</td>')
                md.append(f'<td>{gv_non_aktien_str}</td>')
                md.append(f'<td>{kum_non_aktien_str}</td>')
                md.append(f'<td>{gv_gesamt_str}</td>')
                md.append(f'<td>{kum_gesamt_str}</td>')
                md.append(f'<td>{pdf_link}</td>')
                md.append('</tr>')
            else:
                # Standard Markdown-Zeile
                md.append(f"| {transaction_number} | {value_date_str} | {execution_date_str} | {doc_date_str} | {display_type} | {isin_display} | {shares} | {price_str} | {kurswert_netto} | {gebühren_netto_str} | {ust_str} | {ausmachend_brutto} | {gv_aktien_str} | {kum_aktien_str} | {gv_non_aktien_str} | {kum_non_aktien_str} | {gv_gesamt_str} | {kum_gesamt_str} | {pdf_link} |")
